// ================= GLOBAL =================
const GROUP_ID = document.body.dataset.group; // e.g., "group1"
const workers = ["Ngoronyo", "Sherif", "Scorpion", "Achievers", "Mwea"];

let trucks = JSON.parse(localStorage.getItem(`${GROUP_ID}_trucks`)) || [];
let paymentHistory = JSON.parse(localStorage.getItem(`${GROUP_ID}_history`)) || [];
...
// ================= STORAGE =================
function save() {
  localStorage.setItem(`${GROUP_ID}_trucks`, JSON.stringify(trucks));
  localStorage.setItem(`${GROUP_ID}_history`, JSON.stringify(paymentHistory));
}
...
function renderTable() {
  ...
  trucks.forEach((t, ti) => {
    ...
    t.attendance.forEach((a, wi) => {
      html += `<td class="${a.present ? "" : "zero-attendance"}">
...
function toggle(truckIndex, workerIndex) {
  trucks[truckIndex].attendance[workerIndex].present = !trucks[truckIndex].attendance[workerIndex].present;
  save();
  render();
}
...
function calculateDailyTotals() {
  ...
  trucks.forEach(t => {
    ...
    t.attendance.forEach((a, i) => { if(a.present) totals[workers[i]] += share; });
  });
  return totals;
}
...
function calculateCumulativeTotals() {
  const totals = Object.fromEntries(workers.map(w => [w, 0]));
  paymentHistory.forEach(cycle => { cycle.totals.forEach(([name, amount]) => { totals[name] += amount; }); });
  return totals;
}

function renderTotals() {
  const totals = calculateDailyTotals();
  document.getElementById("totalsOutput").innerHTML = Object.entries(totals)
    .map(([w,v])=>`<p>${w}: <strong>Ksh ${v.toFixed(2)}</strong></p>`).join("");
  renderCharts();
}
...
function renderDailyChart() {
  ...
  dailyChart = new Chart(ctx, {
    type: "bar",
    data: { labels: Object.keys(totals), datasets:[{ label:"Daily Earnings", data:Object.values(totals) }] }
  });
}
...
function renderCumulativeChart() {
  ...
  cumulativeChart = new Chart(ctx, {
    type:"bar",
    data:{ labels:Object.keys(totals), datasets:[{ label:"Cumulative Earnings", data:Object.values(totals) }] }
  });
}

// ================= PAYMENT HISTORY =================
function closePayCycle() {
  if (!trucks.length) return alert("No data to close");
...
function renderHistory() {
  ...
  if (!paymentHistory.length) { el.innerHTML="<p>No payment history yet.</p>"; return; }
  el.innerHTML = paymentHistory.slice().reverse().map(c => `<div class="history-card"><strong>${c.date}</strong>${c.totals.map(t=>`<p>${t[0]}: Ksh ${t[1].toFixed(2)}</p>`).join("")}</div>`).join("");
}
...
function downloadPayslipPDF() {
  ...
  doc.line(20,y,190,y); y+=10; doc.text("Date",20,y); doc.text("Amount (Ksh)",120,y); y+=8;
